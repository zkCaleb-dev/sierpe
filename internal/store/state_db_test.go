...
func commitStates(t *testing.T, s *Store, seq uint32, changes ...StateChange) {
	...
	if err := s.CommitLedger(context.Background(), "testnet", rec, nil, changes, nil, nil); err != nil {
...
func TestStateSnapshotOrderingGuard(t *testing.T) {
	...
	if err := s.CommitBackfillChunk(context.Background(), "testnet", b, nil, []StateChange{old}, nil, nil); err != nil {
...
func TestStateTombstoneBlocksStaleResurrection(t *testing.T) {
	...
	if err := s.CommitBackfillChunk(context.Background(), "testnet", b, nil, []StateChange{created}, nil, nil); err != nil {
		t.Fatalf("CommitBackfillChunk() error = %v", err)
	}
	if _, ok := snapshotValue(t, s, "kA"); ok {
