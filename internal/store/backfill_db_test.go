...
func TestEnsureBackfillLifecycle(t *testing.T) {
	...
	if err := s.CommitBackfillChunk(ctx, "testnet",
		Backfill{ContractID: "CAAA", TargetFrom: 100, NextTo: 3000}, nil, nil, nil); err != nil {
	...
	if err := s.CommitBackfillChunk(ctx, "testnet", done, nil, nil, nil); err != nil {
		t.Fatalf("CommitBackfillChunk() error = %v", err)
	}
...
func TestCommitBackfillChunkPersistsEventsAtomically(t *testing.T) {
	...
	if err := s.CommitBackfillChunk(ctx, "testnet", b, events, nil, nil); err != nil {
		t.Fatalf("CommitBackfillChunk() error = %v", err)
	}
