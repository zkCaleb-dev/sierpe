...
| `sierpe_foreign_transfer_undecodable_total` | counter | Movement-named events from UNWATCHED token contracts that did not decode. | **Never alert.** Anyone can emit an event called transfer carrying anything; this counter exists so that noise stays out of `sierpe_suppressed_transfers_total`. |
