...
| `sierpe_movements_extracted_total` | counter | Token movements attributed to watched contracts that took part in them. | Zero while movements-kind contracts receive funds = attribution problem. |
| `sierpe_foreign_transfer_undecodable_total` | counter | Movement-named events that did not decode, from token contracts whose transfers this instance does not index. | **Never alert** — the population is attacker-controlled: anyone can emit an event called transfer carrying anything. It exists so that noise stays out of `sierpe_suppressed_transfers_total`, which does alert. A non-standard token really moving value into a watched contract also lands here, which is why the movements endpoint documents that it is not a balance. |
